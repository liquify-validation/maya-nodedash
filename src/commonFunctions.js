...
export const getData = async () => {
  ...
  var val = {};
  try {
    const res = await fetch("https://maya-api.liquify.com/maya/api/grabData");
    val = await res.json();
  } catch (error) {
    val = {};
  }
  ...
  const maxTHORHeight = reduceDown(data2, "THOR");
  //const maxTerraHeight = reduceDown(data2, 'TERRA')
  const maxBCHHeight = reduceDown(data2, "BCH");
  const maxDASHHeight = reduceDown(data2, "DASH");
  ...
  const maxXRDHeight = reduceDown(data2, "XRD");
  const maxZCASHHeight = reduceDown(data2, "ZEC");
